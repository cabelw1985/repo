...
def check_paths():
    ...
    if not CONFIG.ADDON_ID == folderpath:
        dialog.ok(CONFIG.ADDONTITLE,
                      '[COLOR {0}]Bitte stelle sicher, dass der Addon-Ordner mit der Addon-ID übereinstimmt.[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]Addon ID:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.ADDON_ID) + '\n' + '[COLOR {0}]Addon Ordner:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, path))
...
def check_skin():
    ...
    if not CONFIG.DEFAULTSKIN == '':
        if os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.DEFAULTSKIN)):
            if dialog.yesno(CONFIG.ADDONTITLE,
                                "[COLOR {0}]Es scheint das der Skin auf [COLOR {1}]{2}[/COLOR] zurückgesetzt wurde.".format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.SKIN[5:].title()) + '\n' + "Willst du den Skin wechseln zu:[/COLOR]" + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.DEFAULTNAME)):
    ...
    if CONFIG.DEFAULTSKIN == '':
        ...
        if len(skinlist) > 0:
            if len(skinlist) > 1:
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "[COLOR {0}]Es scheint das der Skin auf [COLOR {1}]{2}[/COLOR] zurückgesetzt wurde.".format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.SKIN[5:].title()) + '\n' + "Willst du eine Liste aller verfügbaren Skins sehen?[/COLOR]"):
...
def check_sources():
    ...
    if len(match) > 0:
        ...
        progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Überprüfen von Quellen auf defekte URLs[/COLOR]".format(CONFIG.COLOR2))
        for name, path, sharing in match2:
            x += 1
            perc = int(tools.percentage(x, len(match2)))
            progress_dialog.update(perc,
                          '' + '\n' + "[COLOR {0}]Überprüfe [COLOR {1}]{2}[/COLOR]:[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, name) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, path))
        ...
        if len(bad) > 0:
            choice = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]{1}[/COLOR][COLOR {2}] defekte Quellen wurden gefunden.".format(CONFIG.COLOR1, len(bad), CONFIG.COLOR2) + '\n' + "Willst du alle defekten Quellen entfernen oder einzeln auswählen?[/COLOR]",
                                      yeslabel="[COLORlime]Alle entfernen[/COLOR]",
                                      nolabel="[COLORred]Auswählen[/COLOR]")
            if choice == 1:
                remove = bad
            else:
                for name, path, sharing, working in bad:
                    ...
                    if dialog.yesno(CONFIG.ADDONTITLE,
                                        "[COLOR {0}]{1}[/COLOR][COLOR {2}] wurde als nicht funktionierend gemeldet.".format(CONFIG.COLOR1, name, CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, path) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, working),
                                        yeslabel="[COLORlime]Entfernen[/COLOR]",
                                        nolabel="[COLORred]Behalten[/COLOR]"):
            ...
            if len(remove) > 0:
                ...
                removed = len(remove)
                dialog.ok(CONFIG.ADDONTITLE,
                              "[COLOR {0}]Überprüfung der Quellen abgeschlossen.".format(CONFIG.COLOR2) + '\n' + "OK: [COLOR {0}]{1}[/COLOR] | Behalten: [COLOR {2}]{3}[/COLOR] | Entfernt: [COLOR {4}]{5}[/COLOR][/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, alive, CONFIG.COLOR1, kept, CONFIG.COLOR1, removed))
            else:
...
def check_repos():
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]Überprüfe Repos...[/COLOR]'.format(CONFIG.COLOR2))
    badrepos = []
    ...
    while start < sleeptime:
        ...
        progress_dialog.update(perc,
                      '\n' + '[COLOR {0}]Überprüfe: [/COLOR][COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, repolist[start-1].replace(CONFIG.ADDONS, '')[1:]))
        xbmc.sleep(1000)
    if progress_dialog.iscanceled():
        progress_dialog.close()
        logging.log_notify(CONFIG.ADDONTITLE,
    ...
    if len(badrepos) > 0:
        msg = "[COLOR {0}]Unten ist eine Liste von Repos die aktuell nicht erreichbar sind. Das bedeutet nicht zwangsläufig das diese Repos down sind, manche Hosts sind kurzzeitig offline. Um dies zu Überprüfen, führe diese Aktion zu einem späteren Zeitpunbkt nochmal aus.[/COLOR][CR][CR][COLOR {1}]".format(CONFIG.COLOR2, CONFIG.COLOR1)
        msg += '[CR]'.join(badrepos)
        msg += '[/COLOR]'
        window.show_text_box("Defekte Repos anzeigen", msg)
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
