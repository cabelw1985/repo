...
def wipe():
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Berechne Dateien und Ordner".format(CONFIG.COLOR2) + '\n' + '\n' + 'Bitte warten![/COLOR]')
    total_files = sum([len(files) for r, d, files in os.walk(xbmcPath)])
    del_file = 0
    progress_dialog.update(0, "[COLOR {0}]Sammle Informationen der systemspezifischen Addons[/COLOR]".format(CONFIG.COLOR2))
    ...
    progress_dialog.update(0, "[COLOR {0}]Bereinige Dateien und Ordner:".format(CONFIG.COLOR2))
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        for name in files:
            ...
            if name == 'sources.xml' and fold[-1] == 'userdata' and CONFIG.KEEPSOURCES == 'true':
                ...
                progress_dialog.update(int(tools.percentage(del_file, total_files)), '\n' + '[COLOR {0}]Datei: [/COLOR][COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name))
                try:
                    os.remove(os.path.join(root, name))
                except Exception as e:
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        dirs[:] = [d for d in dirs if d not in exclude_dirs]
        for name in dirs:
            progress_dialog.update(100, '\n' + 'Bereinige leere Ordner: [COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name))
...
def fresh_start(install=None, over=False):
    ...
    if over:
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]Willst du Kodi komplett zurücksetzten".format(CONFIG.COLOR2)
                                       +'\n'+"bevor du das Backup installierst?",
                                       nolabel='[COLORred]Nein[/COLOR]',
                                       yeslabel='[COLORlime]Ja[/COLOR]')
    elif install:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Willst du Kodi komplett zurücksetzten".format(CONFIG.COLOR2)
                                       +'\n'+"bevor du [COLOR {0}]{1}[/COLOR] installierst?".format(CONFIG.COLOR1, install),
                                       nolabel='[COLORred]Nein[/COLOR]',
                                       yeslabel='[COLORlime]Ja[/COLOR]')
    else:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Willst du Kodi komplett zurücksetzten?".format(CONFIG.COLOR2), nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
    if yes_pressed:
        wipe()
        ...
        if over:
            ...
            dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Um die Änderungen zu speichern, muss Kodi jetzt beendet werden. Drücken OK, um das Schließen von Kodi zu erzwingen[/COLOR]".format(CONFIG.COLOR2))
            from resources.libs import update
