        ...
        if os.path.exists(LOGINID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Willst du die [COLOR {0}]Login Daten[/COLOR] für [COLOR {1}]{2}[/COLOR] speichern?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
...
def activate_login(who):
    if LOGINID[who]:
        if os.path.exists(LOGINID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} ist aktuell nicht installiert.'.format(LOGINID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
