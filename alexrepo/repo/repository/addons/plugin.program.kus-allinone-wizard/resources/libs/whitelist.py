...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        for folder in sorted(fold2, key=lambda x: x):
            ...
            addonfolds.append(foldername)
        selected = []
        tempaddonnames = ["> [COLORdeepskyblue]Wähle die Addons aus, die du in die Whitelist aufnehmen möchtest[/COLOR]"] + addonnames
        ...
        for item in currentWhite:
            ...
            if id in addonids:
                ...
                tempaddonnames[pos] = "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name)
            else:
                addonids.append(id)
                addonnames.append(name)
                tempaddonnames.append("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name))
        choice = 1
        while choice not in [-1, 0]:
            choice = dialog.select("{0}".format(CONFIG.ADDONTITLE), tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
                break
                ...
                if choice2 in selected:
                    ...
                    tempaddonnames[choice] = "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, addonnames[choice2])
        white_list = []
        if len(selected) > 0:
        ...
        if len(list) > 0:
            msg = "Hier ist eine Liste deiner Whitelist-Addons. Diese Addons (zusammen mit Abhängigkeiten) werden nicht entfernt, wenn ein [COLORred]Frischer Start[/COLOR] oder eine [COLORlime]Build-Installation[/COLOR] durchgeführt wird.[CR][CR]"
            for item in list:
                try:
                    name, id, fold = item
                except Exception as e:
            ...
            window.show_text_box("Addons auf deiner Whitelist", msg)
        else:
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]No items in whitelist[/COLOR]".format(CONFIG.COLOR2))
    elif do == 'import':
        source = dialog.browse(1, '[COLOR {0}]Wähle die zu importierende Whitelist-Datei aus[/COLOR]'.format(CONFIG.COLOR2),
        ...
        source = dialog.browse(3,
                                   '[COLOR {0}]Wähle aus, wohin du die Whitelist-Datei exportieren möchtest[/COLOR]'.format(CONFIG.COLOR2),
                                   'files', '.txt', False, False, CONFIG.HOME)
        logging.log(str(source))
        try:
            xbmcvfs.copy(CONFIG.WHITELIST, os.path.join(source, 'whitelist.txt'))
            dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]Whitelist wurde exportiert nach:[/COLOR]".format(CONFIG.COLOR2)
        ...
        if not dialog.yesno(CONFIG.ADDONTITLE,
                                "[COLOR {0}]Willst du deine Whitelist wirklich löschen?".format(CONFIG.COLOR2)
                                +'\n'+"Dieser Vorgang kann nicht rückgängig gemacht werden.[/COLOR]",
                                yeslabel="[COLORlime]Ja[/COLOR]",
                                nolabel="[COLORred]Nein[/COLOR]"):
            logging.log_notify(CONFIG.ADDONTITLE,
