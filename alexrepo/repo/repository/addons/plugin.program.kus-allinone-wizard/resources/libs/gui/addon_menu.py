...
class AddonMenu:
        ...
        if TEMPADDONFILE:
            ...
            if addons_json:
                ...
                if addons and len(addons) > 0:
                            ...
                            if type.lower() == 'skin':
                                ...
                                try:
                                    ...
                                    if os.path.exists(add):
                                        addonname = "[COLORlime][Installiert][/COLOR] {0}".format(addonname)
                                except:
                                    pass
                    ...
                    if not addons:
                        directory.add_file('Textdatei nicht richtig formatiert!', themeit=CONFIG.THEME3)
                        logging.log("[Addon Menu] ERROR: Invalid Format.")
                    elif len(addons) == 0:
                        directory.add_file("Es wurden noch keine Addons zu diesem Menü hinzugefügt!", themeit=CONFIG.THEME2)
        else:
            logging.log("[Addon Menu] ERROR: URL for Addon list not working.")
            directory.add_file('URL für txt-Datei nicht gültig', themeit=CONFIG.THEME3)
    ...
    def install_addon_from_url(self, plugin, url):
        ...
        self.progress_dialog.create(CONFIG.ADDONTITLE,
                               '[COLOR {0}]Wird heruntergeladen:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                                      CONFIG.COLOR1,
                                                                                                      plugin)
                               +'\n'+''
                               +'\n'+'[COLOR {0}]Bitte warten...[/COLOR]'.format(CONFIG.COLOR2))
        urlsplits = url.split('/')
        ...
        title = '[COLOR {0}]Wird installiert:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                      plugin)
        ...
        self.progress_dialog.update(0, title
                                    +'\n'+''
                                    +'\n'+'[COLOR {0}]Installiere Abhängigkeiten[/COLOR]'.format(CONFIG.COLOR2))
        installed(plugin)
    ...
    def install_addon(self, plugin, urls, over=False):
        ...
        if not over:        
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(plugin)):
                install = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]{1}[/COLOR] bereits installiert. Willst du trotzdem neu installieren?'.format(CONFIG.COLOR1, plugin))
            else:
                install = self.dialog.yesno(CONFIG.ADDONTITLE, 'Willst du [COLOR {0}]{1}[/COLOR] installieren?'.format(CONFIG.COLOR1, plugin))
        else:
            install = True
    ...
    def install_addon_pack(self, name, url):
        ...
        progress_dialog.create(CONFIG.ADDONTITLE,
                      '[COLOR {0}]Wird heruntergeladen:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name)
                      +'\n'+''
                      +'\n'+'[COLOR {0}]Bitte warten...[/COLOR]'.format(CONFIG.COLOR2))
        urlsplits = url.split('/')
        ...
        try:
            ...
            pass
        Downloader().download(url, lib)
        title = '[COLOR {0}]Wird installiert:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name)
        ...
        logging.log_notify("[COLOR {0}]Addon Installer[/COLOR]".format(CONFIG.COLOR1),
                           '[COLOR {0}]{1}: Installiert![/COLOR]'.format(CONFIG.COLOR2, name))
        xbmc.executebuiltin('UpdateAddonRepos()')
        xbmc.executebuiltin('UpdateLocalAddons()')
        xbmc.executebuiltin('Container.Refresh()')
