...
class BuildMenu:
    ...
    def _list_all(self, match, kodiv=None):
        ...
        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
            ...
            if not kodiv or kodiv == int(float(kodi)):
                ...
                directory.add_dir('[COLORdeepskyblue][{0}][/COLOR] {1} (v{2})'.format(float(kodi), name, version), {'mode': 'viewbuild', 'name': name}, description=description, fanart=fanart, icon=icon, menu=menu, themeit=CONFIG.THEME1)
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('[COLORwhite]Kodi Version:[/COLOR] {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
            directory.add_dir('Daten speichern', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
            directory.add_separator()
            directory.add_file('URL für txt-Datei nicht gültig', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('[COLORwhite]Kodi Version:[/COLOR] {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('> Daten speichern', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME1)
        directory.add_separator()
        ...
        if len(match) >= 1:
            if CONFIG.SEPARATE == 'true':
                ...
                if count19 > 0:
                    ...
                    directory.add_file('{0} Matrix Builds [COLORwhite]({1})[/COLOR]'.format(state, count19), {'mode': 'togglesetting',
            ...
            if adultcount > 0:
                directory.add_file('Derzeit gibt es nur Builds für Erwachsene', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME1)
                directory.add_file('Aktivieren Sie "Erwachsene Builds anzeigen" in den Addon-Einstellungen > Sonstiges', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME1)
            else:
                directory.add_file('Derzeit keine Builds angeboten von {0}'.format(CONFIG.ADDONTITLE),
                                   icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME1)
        else:
    ...
    def view_build(self, name):
        ...
        if response:
            ...
            directory.add_file('URL für txt-Datei nicht gültig', themeit=CONFIG.THEME1)
            directory.add_file('{0}'.format(CONFIG.BUILDFILE), themeit=CONFIG.THEME1)
            return

        if not check.check_build(name, 'version'):
            directory.add_file('Fehler beim Lesen der txt-Datei.', themeit=CONFIG.THEME1)
            directory.add_file('{0} wurde nicht in der Build-Liste gefunden.'.format(name), themeit=CONFIG.THEME1)
            return
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            directory.add_separator()
            directory.add_dir('> Daten speichern', {'mode': 'savedata'}, icon=icon, description=description, fanart=fanart, themeit=CONFIG.THEME1)
            directory.add_file('> Build Informationen', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME1)
                               
            if previewcheck:
                directory.add_file('> Videovorschau ansehen', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
            
            if versioncheck:
                directory.add_file(
                    '[I]Das Build wurde für Kodi v.{0} erstellt ! (installiert: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
                    fanart=fanart, icon=icon, themeit=CONFIG.THEME3)
                    
            directory.add_separator('INSTALL')
            directory.add_file('[COLORwhite]>>>[/COLOR] INSTALLATION [COLORwhite]<<<[/COLOR]', {'mode': 'install', 'action': 'build', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME3)
                               
            if guicheck:
                directory.add_file('> guiFix anwenden', {'mode': 'install', 'action': 'gui', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
            ...
            if themecheck:
                ...
                for themename, themeurl, themeicon, themefanart, themeadult, description in match:
                    ...
                    themetitle = themename if not themename == CONFIG.BUILDTHEME else "{0} (Installiert)".format(themename)
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg += "[COLOR {0}]Erwachsenen Inhalt:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, adult)
                msg += "[COLOR {0}]Beschreibung:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, description)

                if extend:
                    msg += "[COLOR {0}]Letztes Update:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, created)
                    msg += "[COLOR {0}]Entpackte Größe:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(extracted))))
                    msg += "[COLOR {0}]Gezippte Größe:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(zipsize))))
                    msg += "[COLOR {0}]Skin Name:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, skin)
                    msg += "[COLOR {0}]Programm-Addons:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, programs)
                    msg += "[COLOR {0}]Video-Addons:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, video)
                    msg += "[COLOR {0}]Musik-Addons:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, music)
                    msg += "[COLOR {0}]Bilder-Addons:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, picture)
                    msg += "[COLOR {0}]Repositories:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, repos)
                    msg += "[COLOR {0}]Scripte:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, scripts)
                    msg += "[COLOR {0}]Abhängigkeiten:[/COLOR] [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, binaries)

                window.show_text_box("Buildinformationen von [B]{0}[/B]".format(name), msg)
            else:
