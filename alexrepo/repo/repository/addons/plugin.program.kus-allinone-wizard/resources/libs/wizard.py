...
class Wizard:
    ...
    def build(self, name, over=False):
        ...
        if warning:
            yes_pressed = self.dialog.yesno("{0} - [COLORred]WARNUNG![/COLOR]".format(CONFIG.ADDONTITLE), '[COLOR {0}]Es besteht die Möglichkeit, dass der Skin nicht richtig angezeigt wird'.format(CONFIG.COLOR2) + '\n' + 'Wenn du ein {0} Build auf Kodi {1} installierst'.format(check.check_build(name, 'kodi'), CONFIG.KODIV) + '\n' + 'Willst du trotzdem [COLOR {0}]{1} v.{2}[/COLOR] installieren?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name, 'version')), nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
        else:
            if over:
                yes_pressed = 1
            else:
                yes_pressed = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Willst du '.format(CONFIG.COLOR2) + '[COLOR {0}]{1} v.{2}[/COLOR] herunterladen und installieren?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name,'version')), nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
        if yes_pressed:
            CONFIG.clear_setting('build')
            buildzip = check.check_build(name, 'url')
            zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')

            self.dialogProgress.create(CONFIG.ADDONTITLE, '[COLOR {0}]Lade Daten:[/COLOR] [COLOR {1}]{2} v.{3}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name, check.check_build(name, 'version')) + '\n' + 'Bitte warten...')
            ...
            title = '[COLOR {0}]Installiere:[/COLOR] [COLOR {1}]{2} v.{3}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name, check.check_build(name, 'version'))
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    ...
                    if yes_pressed:
                        ...
                        window.show_text_box("Installationsfehler", error)
                ...
                self.dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Um die Änderungen zu speichern, muss Kodi beendet werden. Drücke OK, um das Schließen von Kodi zu erzwingen[/COLOR]".format(CONFIG.COLOR2))
    ...
    def gui(self, name, over=False):
        if name == CONFIG.get_setting('buildname'):
            if over:
                ...
                yes_pressed = self.dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Willst du den GuiFix für'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR] anwenden?[/COLOR]'.format(CONFIG.COLOR1, name),
                                   nolabel='[COLORred]Nein[/COLOR]',
                                   yeslabel='[COLORlime]Ja[/COLOR]')
        else:
            yes_pressed = self.dialog.yesno("{0} - [COLORred]WARNUNG![/COLOR]".format(CONFIG.ADDONTITLE),
                               "[COLOR {0}][COLOR {1}]{2}[/COLOR] Build wurde nicht richtig installiert.".format(CONFIG.COLOR2, CONFIG.COLOR1, name) + '\n' + "Willst du den GuiFix trotzdem anwenden?.[/COLOR]",
                               nolabel='[COLORred]Nein[/COLOR]',
                               yeslabel='[COLORlime]Ja[/COLOR]')
        if yes_pressed:
            ...
            self.dialogProgress.create(CONFIG.ADDONTITLE, '[COLOR {0}]Lade GuiFix:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name), '', 'Bitte warten...')
            ...
            title = '[COLOR {0}]Installiere:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, name)
            self.dialogProgress.update(0, title + '\n' + 'Bitte warten...')
    ...
    def theme(self, name, theme='', over=False):
        ...
        if not theme:
            ...
            if response:
                ...
                if len(themes) > 0:
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Das Build [COLOR {1}]{2}[/COLOR] beinhaltet [COLOR {3}]{4}[/COLOR] verschiedene Themes".format(CONFIG.COLOR2, CONFIG.COLOR1, name, CONFIG.COLOR1, len(themes)) + '\n' + "Willst du eines davon jetzt installieren?[/COLOR]",
                                    yeslabel="[COLOR]Ja[/COLOR]",
                                    nolabel="[COLORred]Nein[/COLOR]"):
            ...
            installtheme = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Willst du das Theme installieren:'.format(CONFIG.COLOR2) +' \n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, theme) + '\n' + 'für [COLOR {0}]{1} v{2}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name,'version')),yeslabel="[COLORlime]Ja[/COLOR]", nolabel="[COLORred]Nein[/COLOR]")
        ...
        if installtheme:
            ...
            self.dialogProgress.create(CONFIG.ADDONTITLE, '[COLOR {0}]Lade:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, zipname) +' \n' + 'Bitte warten...')
            ...
            title = '[COLOR {0}]Installiere:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, theme)
            self.dialogProgress.update(0, title + '\n' + 'Bitte warten...')
            percent, errors, error = extract.all(lib, CONFIG.HOME, title=title)
