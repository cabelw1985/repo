        ...
        if response:
            ...
            if repoversion:
                ...
                if repo_response:
                    ...
                    progress_dialog.create(CONFIG.ADDONTITLE, 'Lade Repo' + '\n' + 'Bitte warten...')
...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                           '[COLOR {0}]{2}[/COLOR] [COLOR {1}]Wurde nicht korrekt installiert![/COLOR]'.format(CONFIG.COLOR1,
                                                                                                   CONFIG.COLOR2,
                                                                                                   CONFIG.BUILDNAME)
                           +'\n'+('Installiert: [COLOR {0}]{1}[/COLOR] / '
                            'Fehler: [COLOR {2}]{3}[/COLOR]').format(CONFIG.COLOR1, CONFIG.EXTRACT, CONFIG.COLOR1,
                                                                          CONFIG.EXTERROR)
                           +'\n'+'Willst du es nochmal versuchen?[/COLOR]', nolabel='[COLORred]Nein[/COLOR]',
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]Es sieht so aus, als ob die Skin-Einstellungen nicht auf das Build angewendet wurden.".format(CONFIG.COLOR2)
                          +'\n'+"Leider ist kein GuiFix vorhanden"
                          +'\n'+"Du musst das Build neu installieren und sicherstellen das Kodi richtig beendet wird.[/COLOR]")
            else:
                yes = dialog.yesno(CONFIG.ADDONTITLE,
                                       '{0} wurde nicht korrekt installiert!'.format(CONFIG.BUILDNAME)
                                       +'\n'+'Es sieht so aus, als ob die Skin-Einstellungen nicht auf das Build angewendet wurden.'
                                       +'\n'+'Willst du den GuiFix installieren?',
                                       nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
                if yes:
