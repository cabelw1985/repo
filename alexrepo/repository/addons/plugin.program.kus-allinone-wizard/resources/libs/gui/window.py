...
def highlight_text(msg):
    ...
    for item in matches:
        ...
        msg = msg.replace(string, '[COLORred]{0}[/COLOR]'.format(string))
    msg = msg.replace('WARNING', '[COLORyellow]WARNING[/COLOR]').replace('ERROR', '[COLORred]ERROR[/COLOR]').replace('[NL]', '\n').replace(': EXCEPTION Thrown (PythonToCppException) :', '[COLORred]: EXCEPTION Thrown (PythonToCppException) :[/COLOR]')
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "Schön das du unseren Wizard benutzt![CR]Aktuell hast du kein Build von uns installiert...[CR][CR]Wähle [COLORlime][Build Menü][/COLOR] um ein Build zu installieren[CR] oder [COLORred][Ignorieren][/COLOR] um das Fenster zu schließen.[CR][CR]Viel Spaß mit unseren Builds - [COLORdeepskyblue]Team K.U.S[/COLOR]".format(CONFIG.ADDONTITLE, CONFIG.ADDONTITLE)
            self.msg = CONFIG.THEME1.format(self.msg)
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    ...
    # update.doModal()
    # del update
    msgcurrent = 'Du nutzt bereits das die neuste Version von: '
    msgupdate = 'Update verfügbar für: '
    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)
    current_version = 'Aktuelle Version: v.[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)
    latest_version = 'Neuste Version: v.[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
    ...
    install = xbmcgui.Dialog().yesno(CONFIG.ADDONTITLE, final_msg,
                                     yeslabel='[COLORlime]Installieren[/COLOR]', nolabel='[COLORred]Abbrechen[/COLOR]')
    if install:
...
def show_log_viewer(window_title="LOG anzeigen", window_msg=None, log_file=None, ext_buttons=False):
