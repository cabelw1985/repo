...
def import_save_data():
    ...
    if not os.path.exists(TEMP):
        os.makedirs(TEMP)
    source = dialog.browse(1, '[COLOR {0}]Wählen den Speicherort der SaveData.zip[/COLOR]'.format(CONFIG.COLOR2),
    ...
    overwrite = dialog.yesno(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]Willst du alle Dateien überschreiben oder nach jeder importierten Datei gefragt werden?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[COLORlime]Alle überschreiben[/COLOR]",
                                 nolabel="[COLORred]Fragen[/COLOR]")
            ...
            if os.path.exists(old):
                if overwrite == 1:
                    ...
                    if not dialog.yesno(CONFIG.ADDONTITLE,
                                            "[COLOR {0}]Willst du die aktuelle [COLOR {1}]{2}[/COLOR] Datei ersetzen?".format(CONFIG.COLOR2, CONFIG.COLOR1, item),
    ...
    if os.path.exists(super):
        ...
        if os.path.exists(old):
            cont = dialog.yesno(CONFIG.ADDONTITLE,
                                    "[COLOR {0}]Willst du den aktuellen [COLOR {1}]Super Favourites[/COLOR] Addon_data Ordner mit dem neuen ersetzen?".format(CONFIG.COLOR2, CONFIG.COLOR1),
                                    yeslabel="[COLORlime]Ja[/COLOR]",
                                    nolabel="[COLORred]Nein[/COLOR]")
        else:
            cont = 1
...
def export_save_data():
    ...
    source = dialog.browse(3, '[COLOR {0}]Wählen aus, wohin du die SaveData-Zip exportieren möchtest.[/COLOR]'.format(CONFIG.COLOR2),
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]Die Daten wurden gesichert nach:[/COLOR]".format(CONFIG.COLOR2)
