        ...
        if os.path.exists(TRAKTID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Willst du die [COLOR {0}]Trakt Daten[/COLOR] für [COLOR {1}]{2}[/COLOR] speichern?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
                                    +'\n'+"Addon: [COLORlime]{0}[/COLOR]".format(u)
                                    +'\n'+"Gespeichert:[/COLOR] [COLORred]{0}[/COLOR]".format(su) if not su == '' else 'Gespeichert:[/COLOR] [COLORred]Keine[/COLOR]',
...
def activate_trakt(who):
    if TRAKTID[who]:
        if os.path.exists(TRAKTID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} ist aktuell nicht installiert.'.format(TRAKTID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
