        ...
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "Willst du die [COLOR {0}]Debrid Daten[/COLOR] für [COLOR {1}]{2}[/COLOR] speichern?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
    ...
    if DEBRIDID[who]:
        if os.path.exists(DEBRIDID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE,
                          '{0} ist aktuell nicht installiert.'.format(DEBRIDID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
