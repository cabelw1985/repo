            ...
            for root, dirs, files in os.walk(CONFIG.PACKAGES):
                ...
                if file_count > 0:
                    ...
                    if over:
                        ...
                        yes = dialog.yesno("[COLOR {0}]Package Dateien löschen[/COLOR]".format(CONFIG.COLOR2), "[COLOR {0}]{1}[/COLOR] Dateien gefunden / [COLOR {2}]{3}[/COLOR].".format(CONFIG.COLOR1, str(file_count),CONFIG.COLOR1, size) + '\n' + "Willst du diese bereinigen", nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
                    if yes:
                        for f in files:
...
def clear_archive():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Willst du den Archive-Cache bereinigen?[/COLOR]'.format(CONFIG.COLOR2),
...
def clear_function_cache(over=False):
    ...
    if not over:
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Willst du den Resolver-Cache bereinigen?[/COLOR]'.format(CONFIG.COLOR2),
                            nolabel='[COLORred]Nein[/COLOR]',
                            yeslabel='[COLORlime]Ja[/COLOR]'):
            clear = True
    else:
...
def clear_crash():
    ...
    if len(files) > 0:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Willst du den Crash LOG bereinigen?'.format(CONFIG.COLOR2)
                            +'\n'+'[COLOR {0}]{1}[/COLOR] Dateien gefunden[/COLOR]'.format(CONFIG.COLOR1, len(files)),
                            yeslabel="[COLORlime]Ja[/COLOR]",
                            nolabel="[COLORred]Nein[/COLOR]"):
            for f in files:
                os.remove(f)
...
def total_clean():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Willst du Cache, Packages und Thumbnails bereinigen?[/COLOR]'.format(CONFIG.COLOR2),
                        nolabel='[COLORred]nein[/COLOR]',
                        yeslabel='[COLORlime]Ja[/COLOR]'):
        clear_archive()
        clear_cache()
...
def clear_thumbs(type=None):
    ...
    if type is not None:
        ...
        choice = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Willst du die {1} und die Thumbnails bereinigen?'.format(CONFIG.COLOR2, latest) + '\n' + "Die Thumnails werden beim nächsten Kodi Start wieder aktualisiert.[/COLOR]", nolabel='[COLORred]Nein[/COLOR]', yeslabel='[COLORlime]Ja[/COLOR]')
    if choice == 1:
        try:
...
def remove_addon(addon, name, over=False, data=True):
    ...
    if over:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                               '[COLOR {0}]Willst du das Addon wirklich löschen?:'.format(CONFIG.COLOR2)
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Willst du alle Addon Daten löschen?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[COLORlime]Ja[/COLOR]',
                            nolabel='[COLORred]Nein[/COLOR]'):
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Willst du alle Addon Daten von deinstallierten Addons löschen?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[COLORlime]Ja[/COLOR]',
                            nolabel='[COLORred]Nein[/COLOR]'):
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Willst du alle leeren Ordner im Userdata Ordner löschen?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[COLORlime]Ja[/COLOR]',
                            nolabel='[COLORred]Nein[/COLOR]'):
        ...
        if addon in CONFIG.EXCLUDES:
            ...
            if dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Willst du die Addon Daten löschen für:[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, addon), yeslabel='[COLORlime]Ja[/COLOR]', nolabel='[COLORred]Nein[/COLOR]'):
...
def remove_addon_menu():
    ...
    if len(addonnames) == 0:
        ...
        return
    selected = dialog.multiselect("Wähle Addons zum Löschen.".format(CONFIG.ADDONTITLE), addonnames)
    if not selected:
        return
    if len(selected) > 0:
        ...
        dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Um die Änderungen zu speichern muss Kodi geschlossen werden.[CR]Klicke OK um Kodi zu schließen![/COLOR]".format(CONFIG.COLOR2))
