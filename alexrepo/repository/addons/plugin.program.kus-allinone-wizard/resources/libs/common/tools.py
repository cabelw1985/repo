...
def kill_kodi(msg=None, over=None):
    if over:
        ...
        if not msg:
            msg = '[COLOR {0}]Du bist dabei Kodi zu schließen. Willst du weiter machen?[/COLOR]'.format(CONFIG.COLOR2)
...
def convert_special(url, over=False):
    ...
    start = 0
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Konvertiere Pfade zu special://".format(CONFIG.COLOR2) + "\n" + "Bitte warten...[/COLOR]")
    for root, dirs, files in os.walk(url):
        for file in files:
            start += 1
            perc = int(percentage(start, total))
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                progress_dialog.update(perc, "[COLOR {0}]Überprüfe: [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, root.replace(CONFIG.HOME, '')) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, file) + '\n' + "Bitte warten...[/COLOR]")
...
def reload_fix(default=None):
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]WARNUNG: Manchmal führt das Neuladen des Profils zum Absturz von Kodi. Während Kodi das Profil neu lädt, drücke bitte keine Tasten![/COLOR]".format(CONFIG.COLOR2))
...
def ascii_check(use=None, over=False):
    ...
    if use is None:
        source = dialog.browse(3,
                                   '[COLOR {0}]Wählen den Ordner aus, den du überprüfen möchtest[/COLOR]'.format(CONFIG.COLOR2),
                                   'files', '', False, False, CONFIG.HOME)
        if over:
            yes = 1
        else:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Willst du alle Dateinamen mit Sonderzeichen löschen oder einfach nur überprüfen und die Ergebnisse im Protokoll anzusehen?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
                                   yeslabel='[COLORlime]Löschen[/COLOR]',
                                   nolabel='[COLORred]Überprüfen[/COLOR]')
    else:
        source = use
        yes = 1
    ...
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            progress_dialog.update(prog2, "[COLOR {0}]Auf Nicht-ASCII-Dateien prüfen".format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file) + '\n' + 'Bitte warten...[/COLOR]')
            try:
                file.encode('ascii')
    ...
    if total > 0:
        ...
        if yes:
            if use:
                ...
                window.show_text_box("Gelöschte ASCII Files",
                                  "[COLORyellow]{0} Dateien gelöscht:[/COLOR]\n {1}\n\n[COLORyellow]{2} Dateien fehlgeschlagen:[/COLOR]\n {3}".format(f1, msg, f2, msg2))
        else:
            window.show_text_box("Gefundene ASCII Files", "[COLORyellow]{0} Dateien gefunden:[/COLOR]\n {1}".format(f1, msg))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        if response.status_code == 401:
            retry = dialog.yesno(CONFIG.ADDONTITLE, 'Entweder der Benutzername oder das Kennwort sind ungültig. Willst du es nochmal versuchen?', yeslabel='[COLORlime]Ja[/COLOR]', nolabel='[COLORred]Nein[/COLOR]')
