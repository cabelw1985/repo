    ...
    if response:
        ...
        if ver > CONFIG.ADDON_VERSION:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Es gibt eine neue Version von {1}!'.format(CONFIG.COLOR2, CONFIG.ADDONTITLE)
                                   +'\n'+'Willst du [COLOR {0}]v.{1}[/COLOR] herunterladen & installieren?[/COLOR]'.format(CONFIG.COLOR1, ver),
            ...
            if yes:
                ...
                progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]Lade Daten...'.format(CONFIG.COLOR2)
                                        +'\n'+''
                                        +'\n'+'Bitte warten...[/COLOR]')
                ...
                xbmc.sleep(2000)
                progress_dialog.update(0, '\n'+"Installiere {0} Update".format(CONFIG.ADDONTITLE))
...
def toggle_addon_updates():
    ...
    setting = '"general.addonupdates"'
    selected = 0
    options = ['Updates automatisch installieren', 'Benachrichtigen, aber keine Updates installieren', 'Nie auf Updates überprüfen']
